...
if (!class_exists('si_Email_WriterOutfittersLicense')) {
    class si_Email_WriterOutfittersLicense
    {
        /**
        ...
        public static function isValid($thisModule = null, $user_id = null, $force_validation = false)
        {
            ...
            $validated = si_Email_WriterOutfittersLicense::doValidate($thisModule);
        ...
        public static function getKey($thisModule = null)
        {
            ...
            if (empty($key) && !empty($sugar_config['outfitters_licenses']) && !empty($sugar_config['outfitters_licenses'][$outfitters_config['shortname']])) {
                ...
                //stash this key in the config table
                if (!empty($key)) {
        ...
        public static function validate()
        {
            ...
            $validated = si_Email_WriterOutfittersLicense::doValidate($currentModule, $_REQUEST['key']);
        ...
        public static function doValidate($thisModule, $key = null)
        {
            ...
            if (empty($response['success']) || $response['success'] !== true) {
                $GLOBALS['log']->fatal('si_Email_WriterOutfittersLicense::doValidate() failed: ' . print_r($response, true));
            }
        ...
        public static function change()
        {
            ...
            if (empty($response['success']) || $response['success'] !== true) {
                header('HTTP/1.1 400 Bad Request');
                $GLOBALS['log']->fatal('si_Email_WriterOutfittersLicense::change() failed: ' . print_r($response, true));
            } else {
                require_once('modules/Administration/Administration.php');
                $administration = new Administration();
                $administration->saveSetting('SugarOutfitters', 'lic_' . $outfitters_config['shortname'], $_REQUEST['key']);
        ...
        public static function add()
        {
            ...
            $response = si_Email_WriterOutfittersLicense::doValidate($currentModule);
    ...
    class SugarOutfitters_API
    {
        private static function get_default_payload($module, $custom_data = array())
        {
            ...
            if (empty($custom_data['key'])) {
                $key = si_Email_WriterOutfittersLicense::getKey($module);
                $data['key'] = empty($key) ? false : $key;
            } else {
